...
export function hasCurrentDemandEvidence(e,now=Date.now()) {
  return !!(e && e.qualified===true && e.sourceUrl && e.summary && recent(e.observedAt,7,now) && recent(e.periodEnd,7,now));
}
...
function computeAssessment(p, catalog, now) {
  ...
  else if(own || (currentReview && (reviewed.currentlyCarried===true || reviewed.equivalentCurrentlyCarried===true || reviewed.temporarilyOutOfStock===true))) {status='Target listing recorded';tone='benchmark';reason=currentReview?reviewed.reason:'Excluded from whitespace: this product or a confirmed equivalent is recorded at Target. A temporarily sold-out item still belongs to its assortment.';next=currentReview&&reviewed.nextStep?reviewed.nextStep:'Use it as a benchmark. Recheck the current Target listing, seller, price and pack; do not present it as new whitespace.';}
  ...
  const demandChecked=hasCurrentDemandEvidence(p.demandEvidence,now);
